<template>
    ...
    >
      <div class="bg-white p-5">
        ...
        <p class="m-2">Data: {{ formatDate(data.date) }}</p>
  
        <p class="m-2">IDIngresso: {{ data.reference_number }}</p>
  
        <p class="m-2">
          Assentos sem pipoca e bebidas: {{ data.seats_without }} seats
        </p>
  
        <p class="m-2">
          Assentos com pipoca e bebidas: {{ data.seats_with }} seats
        </p>
        ...
        <p class="m-2">Preço Total: ${{ data.total }}.00</p>
  
        <router-link to="/">
          <button class="m-2 p-3 text-white bg-black">Pronto</button>
        </router-link>
      </div>
    </div>
  </template>
